    ...
    wrapper.listenForChatMsg(async ({ userId, msg }) => {
     ...
     if (msg.tokens[0].v === "!help") {
        await wrapper.sendRoomChatMsg([
          {
            t: "text",
            v:
              "𝗟𝗶𝘀𝘁 𝗼𝗳 𝗖𝗼𝗺𝗺𝗮𝗻𝗱𝘀: !8ball ● !slap (user) ● !yeet (user) ● !say (words) ● !joke ● !howgay ● !owoify (text) ● !rolldice ● !findcute ● !whoami ● !whereami ● !creator"
          }
        ]);
        } 
      ...
      if (msg.tokens[0].v === "!requestspeaker") {
       ...
       if (msg.tokens[0].v === "!mutebot") {
        if (msg.userId === "b4c3169e-9ae4-4fb8-a4a2-bdfce065a6a5") {
          await wrapper.sendRoomChatMsg([
            {
              t: "text",
              v: "🔊 BenBot has been muted. "
            }
          ]);
        } else {
          await wrapper.sendRoomChatMsg([
